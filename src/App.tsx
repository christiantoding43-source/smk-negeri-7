...
import { RPPData, JenjangPendidikan } from './types';
import { getSavedRPPs, saveRPP, deleteRPP, duplicateRPP } from './utils/storage';
import { exportRPPToWord } from './utils/wordExport';
import { Sparkles, FileText, CheckCircle2, BookOpen, Layers, ShieldCheck, Download, Printer, Lightbulb } from 'lucide-react';
...
export default function App() {
  ...
  const handleGeneratedSuccess = (newRPP: RPPData) => {
    const updatedList = saveRPP(newRPP);
    setSavedRPPs(updatedList);
    setCurrentRPP(newRPP);
    setViewMode('view');
    showToast('RPP & Modul Ajar lengkap berhasil disusun oleh AI!');
  };

  const handleSaveEdited = (updated: RPPData) => {
  ...
  const handleDuplicate = (rpp: RPPData) => {
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="space-y-1.5 max-w-2xl">
              ...
              <p className="text-xs sm:text-sm text-slate-200 leading-relaxed">
                ...
                Lengkap dengan Capaian Pembelajaran, Sintaks Sintetik, Asesmen Diagnostik/Formatif/Sumatif HOTS, Rubrik KKTP, LKPD, dan siap diekspor ke Microsoft Word (.doc) atau dicetak PDF.
              </p>
            </div>
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2.5 w-full md:w-auto">
              ...
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 text-xs sm:text-sm font-semibold text-white bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl transition-all"
              >
